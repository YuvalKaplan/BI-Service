import log
from bt.object import provider, categorize_etf, ticker
...
def run() -> int:
    try:
        ...
        log.record_status(f"Running Categorize Tickers - will proccess {len(etfs)} items.")
        ...
        log.record_status(f"Finished Categorize Tickers run on {len(etfs)} items.\n")
        log.record_status(f"Processed {total_symbols} tickers.\n")
        return total_symbols

    except Exception as e:
        log.record_error(f"Error in categorize tickers run: {e}")
        raise e