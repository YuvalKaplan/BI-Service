...
def record_problem(provider: provider.Provider, etf: provider_etf.ProviderEtf, error: str, message: str | None, problem_etfs: list[str]):
...
def run(todey: date) -> tuple[int, int, list[str]]:
    try:
        ...
        log.record_status(f"Running Best Ideas Generator - will proccess {len(providers)} providers.")
            ...
            for pe in pe_list:
                try:
                    ...
                    if len(available_holding_dates) == 0:
                        record_problem(provider=p, etf=pe, error="No holdings have been downloaded for the past week", message=None, problem_etfs=problem_etfs)
                        continue
                    ...
                    if latest_common_date is None:
                        ...
                        record_problem(provider=p, etf=pe, error="Data sources out of sync", message=message, problem_etfs=problem_etfs)
                        continue
                    ...
                    if len(holdings) < MIN_HOLDINGS:
                        ...
                        record_problem(provider=p, etf=pe, error=error, message=None, problem_etfs=problem_etfs)
                        continue
                    ...
                    if len(values) < int(0.9 * len(holdings)):
                        ...
                        record_problem(provider=p, etf=pe, error="Too many prices missing (less than 90 percent of holdngs)", message=message, problem_etfs=problem_etfs)
                        continue
                    ...
                    record_problem(provider=p, etf=pe, error=f"{e}", message=None, problem_etfs=problem_etfs)
